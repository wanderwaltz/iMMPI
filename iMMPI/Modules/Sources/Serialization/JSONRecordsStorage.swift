...
public let kJSONRecordStorageDirectoryDefault = "JSONRecords"
public let kJSONRecordStorageDirectoryTrash = "JSONRecords-Trash"

public final class JSONRecordsStorage {
    public var trashStorage: JSONRecordsStorage?

    public init(
        directoryName: String = kJSONRecordStorageDirectoryDefault,
        indexSerialization: JSONRecordIndexSerialization = .init(),
        recordSerialization: JSONRecordSerialization = .init(),
...
extension JSONRecordsStorage: RecordStorage {
    public var all: [RecordProtocol] {
        return elements.compactMap({ $0.record })
    }


    public func add(_ record: RecordProtocol) throws {
        let element = Element()
    ...
    public func update(_ record: RecordProtocol) throws {
        try store(element(for: record))
    }


    public func remove(_ record: RecordProtocol) throws {
        try remove(element(for: record))
    }


    public func contains(_ record: RecordProtocol) -> Bool {
